...
async function addBim() {
  ...
  try {
    ...
    await ladeEintraege(); // Bei Fehler echte Daten neu laden
  }
...
// Setzt alle Formularfelder zurück nach dem Eintragen
...
function render() {
  ...
  document.getElementById("s-missing").textContent     = 62 - total;
...
document.addEventListener("keydown", e => {
  const appSichtbar = document.getElementById("seite-app")
                               .classList.contains("aktiv");
  if (e.key === "Enter" && appSichtbar) addBim();
});
